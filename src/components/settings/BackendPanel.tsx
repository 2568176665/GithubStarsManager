...
import { buildAppStateSnapshot } from '../../services/appStateSnapshot';
...
export const BackendPanel: React.FC<BackendPanelProps> = ({ t }) => {
  ...
  const handleSyncToBackend = async () => {
    ...
    try {
      ...
      const results = await Promise.allSettled([
        ...
        backend.syncSettings(buildAppStateSnapshot(useAppStore.getState())),
      ]);
  ...
  const handleSyncFromBackend = async () => {
    ...
    try {
      ...
      // Preserve local analysis fields when an older backend snapshot omits them.
      setRepositories(mergeRepositoriesPreservingLocalMetadata(repoData.repositories, repositories));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="hidden p-4 bg-background dark:bg-muted/40 rounded-lg border border-border dark:border-border">
      ...
      <Button
        ...
        className="flex items-center justify-center space-x-2 px-4 py-2 bg-primary text-primary-foreground rounded-lg hover:bg-primary/90 transition-colors disabled:opacity-50"
      >
        {status === 'checking' ? <RefreshCw className="w-4 h-4 animate-spin" /> : <TestTube className="w-4 h-4" />}
        <span>{t('直接连接', 'Connect')}</span>
      </Button>
